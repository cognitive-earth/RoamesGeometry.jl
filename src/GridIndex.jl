...
function closestPointInCell(pos::SVector{3,Float64}, grid::GridIndex, gx::Int64, gy::Int64, points::AcceleratedArray{<:Any, <:Any, <:Any, <:GridIndex}, bounds::Vector{Float64}, minDist²::Float64, maxDist²::Float64, best::Int64, excludeStart::Int64, excludeEnd::Int64)
    for i ∈ grid[gx, gy]
        j = grid.index[i]
...
"""
    ...
    densities or index resolutions. The position pos is assumed not to exist inside points. 
"""
function findClosestPointIndex(pos::SVector{3,Float64}, points::AcceleratedArray{<:Any, <:Any, <:Any, <:GridIndex}, minDist::Float64, maxDist::Float64, excludeStart::Int64, excludeEnd::Int64)
    ...
    found, best, maxDist² =closestPointInCell(pos, grid, firstCellX, firstCellY, points, bounds, minDist², maxDist², best, excludeStart, excludeEnd)
    if found
        return best, sqrt(maxDist²)
    end

    # Find distances to edges of first cell, if all further than best match found so far return
    ...
    if (gx_min < secondCellX < gx_max) && (gy_min < secondCellY < gy_max) 
        found, best, maxDist² = closestPointInCell(pos, grid, secondCellX, secondCellY, points, bounds, minDist², maxDist², best, excludeStart, excludeEnd)
        if found
            return best, sqrt(maxDist²)
        end
    ...
    @inbounds for gy ∈ gy_min:gy_max
        for gx ∈ gx_min:gx_max
            ...
            found, best, maxDist² = closestPointInCell(pos, grid, gx, gy, points, bounds, minDist², maxDist², best, excludeStart, excludeEnd)
            if found
                return best, sqrt(maxDist²)
            end
        end
